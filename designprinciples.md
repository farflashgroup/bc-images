...
- **Installed font aliases:** Include common local family aliases in CSS font stacks and fallback checks, especially `"TestSöhne"` for Test Söhne and `"TestFoundersGrotesk"`, `"Test Founders Grotesk Text"`, and `"TestFoundersGroteskText"` for Test Founders Grotesk, because Windows installations may expose those alternate names to the browser.
...
- **Cross-platform legibility:** Assume every graphic will be viewed on phones and in compressed thumbnails. Scale **all** tiers together (header, labels, body, stats, footer): tighten padding, margins, and gaps before shrinking type below the floors above. Numeric callouts should still visually dominate adjacent prose.
